...
interface TokenStatsContextType {
  ...
  loading: boolean;
  addTokenUsage: (model: string, inputTokens: number, outputTokens: number, requestType?: string, cached?: boolean) => void;
  clearStats: () => void;
  getStatsForPeriod: (days: number) => TokenStatsData | null;
}

const TokenStatsContext = createContext<TokenStatsContextType | undefined>(undefined);
...
export function TokenStatsProvider({ children }: TokenStatsProviderProps) {
  ...
  const value: TokenStatsContextType = {
    stats,
    loading,
    addTokenUsage,
    clearStats,
  ...
  return (
    <TokenStatsContext.Provider value={value}>
      {children}
...
export function useTokenStatsContext() {
  ...
  if (context === undefined) {
    throw new Error('useTokenStatsContext must be used within a TokenStatsProvider');
  }
  return context;
}
